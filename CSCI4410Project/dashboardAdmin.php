...
<?php
    ...
    echo "<a href='logout.php'>Logout</a><br>"; //this will be our logout hyperlink (added break for spacing)
...
<form method="post">
    ...
    <button name="update_book">Update Book Info</button> <!-- -->
...
if (isset($_POST['display_booklist'])) { //IF FUNCTION FOR DISPLAYINGA ALL BOOKLIST WITHOUT IMAGE
    $sql = "SELECT book_id, title, author, ISBN, category, total_copies, available copies FROM books"; //save query to $sql
...
if (isset($_POST['add_user'])) { //first we display a form for entering new user information
    ...
    echo '<button type="submit" name="add_user_submit">Add Book</button>';
    echo '</form><br>';
